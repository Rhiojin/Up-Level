...
//		}
	...
	void Jump()
	{
		soundManager.instance.PlayClip("jump",0.35f);
		canJump = false;
		anim.Play("pc_Jump");
	...
	void WeightlessJump()
	{
		soundManager.instance.PlayClip("swim",0.2f);
		thisRigidbody.AddForce(upVector*weightLessForce);
	}

	IEnumerator RocketBoost()
	{
		soundManager.instance.PlayClip("rocket",0.2f);
		camScript.Reset();
		trail.enabled = true;
...
//		anim.Play("pc_Jump");
	...
	{
		soundManager.instance.PlayClip("whoosh");
	...
	{
		soundManager.instance.PlayClip("enterCannon");
		inDonkeyCannon = true;
	...
	{
		soundManager.instance.PlayClip("cannon");
	...
	IEnumerator Death()
	{
		...
		if(isFacingRight)
		{
			thisRigidbody.AddForce(upVector*2000 + -forwardVector2*1000);
		}
		else
		{
			thisRigidbody.AddForce(upVector*2000 + forwardVector2*1000);
		...
		if((PlayerPrefs.GetInt("coins",0) >= 100 || AdManager.AdColony_AdAvailable())&& respawnTransform != null && !respawned)
		{
			...
			respawnPeg.position = respawnTransform.position;
		}
...
//			stompParticle.transform.parent = null;
	...
	IEnumerator ActualRespawn()
	{
		...
		respawnPeg.position = transform.position - new Vector3(0,-100,0);
		anim.Play("pc_Run");
		transform.position = respawnTransform.position+Vector3.up*3.5f;
		stompParticle.Emit(3);
		stompParticle.Stop();
	...
	void ClearResapwnArea()
	{
		Collider2D [] allColl2D = Physics2D.OverlapCircleAll(new Vector2(respawnTransform.position.x, respawnTransform.position.y), 10, 1 << 9);
		for (int i=0; i<allColl2D.Length; i++) 
		{
			Destroy(allColl2D[i].gameObject);
		}
	}
	...
	void OnCollisionEnter2D(Collision2D colEnt)
	{
		if(colEnt.collider.CompareTag("ground"))
		{
			if(ButtStomping)
			{
				soundManager.instance.PlayClip("buttStomp");
			...
			{
				soundManager.instance.PlayClip("poof");
				anim.Play("pc_Run");
		...
		if(colEnt.collider.CompareTag("enemy"))
		{
			...
			{
				soundManager.instance.PlayClip("punch");
				if(ButtStomping)
				{
					camScript.StartCamRock(transform.position.x);
					stompParticle.Emit(5);
		...
		if(colEnt.collider.CompareTag("breakable"))
		{
			if(ButtStomping || RocketBoosting || ShoulderBashing)
			{
				soundManager.instance.PlayClip("crunch",0.2f);
	...
	void OnTriggerEnter2D(Collider2D col)
	{
		...
		{
			soundManager.instance.PlayClip("coinSound", 0.35f);
